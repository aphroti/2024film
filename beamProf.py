...
image_path = "W:\\RadOnc\\Research\\2024_flash\\Flash_film_scan\\test071.tif"
...
# Save the OD profiles along with their distances in mm to text files
np.savetxt('od_x_profile.txt', np.column_stack((x_mm, od_x_profile)), header='Distance (mm), OD (X Direction)', fmt='%10.5f', delimiter=',')
np.savetxt('od_y_profile.txt', np.column_stack((y_mm, od_y_profile)), header='Distance (mm), OD (Y Direction)', fmt='%10.5f', delimiter=',')
...
plt.title('Optical Density Profile along X and Y Directions (in mm)')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
plt.title('Original Image with Profile Lines')
plt.axis('off')
